...
// Add type for profession descriptions
...
export default function ProfessionsAdmin() {
  ...
  // Fetch all professions for the list
  React.useEffect(() => {
    setListLoading(true);
  ...
  React.useEffect(() => {
    setLoading(true);
    setError("");
    fetch(`/api/profession?role=${encodeURIComponent(role)}`)
      .then(async res => {
        const data = await res.json();
  ...
  return (
    <Box sx={{ width: '90vw', maxWidth: 1500, mx: "auto", mt: 4, display: 'flex', gap: 4 }}>
      <Card variant="outlined" sx={{ minWidth: 320, maxWidth: 350, p: 2, boxShadow: 'md', flex: '0 0 350px', height: 'fit-content' }}>
        <Typography level="h4" sx={{ mb: 2 }}>All Professions</Typography>
        {listLoading ? <CircularProgress /> : (
          <List>
            {professions.length === 0 && <Typography level="body-sm">No professions found.</Typography>}
            {professions.map(prof => (
              <ListItem key={prof.roleName}>
                <ListItemButton selected={role === prof.roleName} onClick={() => setRole(prof.roleName)}>
                  <Box>
                    <Typography level="body-md" fontWeight={600}>{prof.roleName}</Typography>
                    <Typography level="body-sm" color="neutral" sx={{ fontStyle: 'italic' }}>{getPreview(prof.description)}</Typography>
                  </Box>
                </ListItemButton>
              </ListItem>
            ))}
            ...
            {ROLES.filter(r => !professions.some(prof => prof.roleName === r)).map(r => (
              <ListItem key={r}>
      ...
      <Box sx={{ flex: 1 }}>
        <Card variant="outlined" sx={{ p: 4, boxShadow: 'lg' }}>
          <Typography level="h3" sx={{ mb: 2 }}>Professions</Typography>
          ...
          {loading ? <CircularProgress /> : (
            <Box>
              <Textarea
                ...
                placeholder="Enter Markdown description for this profession..."
